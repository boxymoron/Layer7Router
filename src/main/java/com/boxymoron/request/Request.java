		...
		for(int i=0;i<remaining && prev+2 < remaining;i++) {
			...
			if(chars[i] == '\r') {
				...
				if(prev == 0) {
					...
					//Arrays.equals(Arrays.copyOfRange(chars, prev, prev+HOST_LEN-1), HOST);
					if(host == null && startsWith(chars, HOST, prev)) {
						final int start = prev+HOST_LEN;
						host = new String(chars, start, i-start);
					}else if(connection == null && startsWith(chars, CONNECTION, prev)) {
						final int start = prev+CONNECTION_LEN;
						connection = new String(chars, start, i-start);
						if(connection.equals("keep-alive")) {
							keepalive = true;
						}
						/*if(startsWith(chars, KEEP, prev+CONNECTION_LEN)) {
							keepalive=true;
						}*/
					}else if(expect == null && startsWith(chars, EXPECT, prev)) {
						final int start = prev+EXPECT_LEN;
						expect = new String(chars, start, i-start);
					}else if(content_length == -1 && (startsWith(chars, CONTENT_LENGTH, prev) || startsWith(chars, CONTENT_LENGTH2, prev))) {
