...
public final class Layer7RouterFrontend {
	...
	public static void main(String[] args) throws Exception {
		...
		final InetSocketAddress backendAddr = new InetSocketAddress("192.168.1.150", 80);
		for(int ip=218; ip<=218;ip++) {
			for(int port=13000; port<=65000;port++) {
				final InetSocketAddress addr = new InetSocketAddress("192.168.1."+ip, port);
				//System.out.println(addr+" Connecting to "+backendAddr);
				final IoFuture<StreamConnection> future = worker.openStreamConnection(addr, backendAddr, new ChannelListener<StreamConnection> () {
					@Override
